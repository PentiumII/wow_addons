...
function offlineRaidersFrame_OnLoade()
	...
	RaidersShowFrame.text:SetPoint("TOP",5,-50)
...
function toggleOfflineRaidersSent()
	if EncounterJournal.encounterID then
		...
		if raidersText and encounterName then
			...
			if IsInRaid() then
				SendChatMessage(encounterName, "raid");
				SendChatMessage(raidersText, "raid");
			elseif IsInGroup() then
				SendChatMessage(encounterName, "party");
				SendChatMessage(raidersText, "party");
			else
