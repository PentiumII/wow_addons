...
function RaidAlerter_SET_FUNC_ResetToDefaut(arg)
	...
	RaidAlerter_SET_FUNC_Paladin_Intervention(1);
	...
	RaidAlerter_SET_FUNC_SpellIDAndCaster(1);
--	RaidAlerter_Attack_Frame:Show()
	RaidAlerter_Check_CTRAORA_LA_TANKS();
	RaidAlerter.AddMsg("|cFF20FF20"..RAL_TEXT_FUNC_48);
end
...
--	RaidAlerter_SET_FUNC_Party_Alerter(0);
	...
	RaidAlerter_SET_FUNC_Paladin_Intervention(0);
	...
	for i = 1,GetNumGroupMembers() do
		...
		if subgroup <= RaidAlerter_SET.RAIDMODE_MAX_GROUP then
			...
			while UnitBuff(unit, j) do
				...
				--new add 获取buffid
				local BuffTEXT,_,_,_,_,_,_,_,_,_,Buffid = UnitBuff(unit, j);
	...
	if RaidAlerter_SET.OnOff then
		...
		if GameVER >= 40100 then		--3.35-4.1兼容过渡
			if GameVER >= 40200 then
				local tmpone,tmptwo;
				timestamp, eventtype, _, srcGUID, srcName, srcFlags, tmpone, dstGUID, dstName, dstFlags, tmptwo, spellId, spellName, spellSchool, auraType, amount, otherPara1, otherPara2 = ...;
			else
