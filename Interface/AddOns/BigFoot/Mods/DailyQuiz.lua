--thanks to [http://bbs.ngacn.cc/read.php?tid=6526010]
...
do
	...
	f:SetScript("OnEvent",function(self,event,...)
			...
			if npc~="资深历史学家伊夫琳娜" then
				return
			end
			local body=GossipGreetingText:GetText()
			if body == "答对了！" or body == "答错了！" then return end
			local questbody = string.match(body,"那就让我们来测试一下你的历史知识吧！(.+)")
			--print("资深历史学家伊夫琳娜 actived:",questbody)
			for _,v in pairs(pdata) do
				local match=string.match(v,questbody)
				if match then
					local choose=string.match(v,"!(%a)")
					local chossebutton
					if choose=="A"      then choosebutton=GossipTitleButton3
					elseif choose=="B" then choosebutton=GossipTitleButton4
					elseif choose=="C" then choosebutton=GossipTitleButton5
					elseif choose=="D" then choosebutton=GossipTitleButton6
					end
