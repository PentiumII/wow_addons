-- Modify by Mini_Dragon to fit 6.0 API
...
local function mini_array_v(...) -- Mini_Dragon add
	t = {}
	for i=1, select("#", ...), 2 do
		k = select(i, ...)
		v = select(i+1, ...)
...
local function activate( newInstance, oldInstance )
	...
	if not ( oldInstance and oldInstance.HarvestedMapData.VERSION == HARVESTED_DATA_VERSION ) then
		...
		newInstance.ContinentList = mini_array_v( GetMapContinents() ); --Mini_Dragon modify, check line 1173
		for C in pairs(newInstance.ContinentList) do
			local zones = mini_array_v( GetMapZones(C) ); --Mini_Dragon modify
