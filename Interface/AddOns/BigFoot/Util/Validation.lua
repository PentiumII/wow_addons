...
function e:CHAT_MSG_ADDON(...)
	...
	if(filter =="BF_VER_CHK") then
		if message ==BIGFOOT_VERSION_QUERY then
			SendAddonMessage("BF_VER_CHK",BF_VERSION_CHECKSUM.."|"..BIGFOOT_VERSION,'WHISPER',sender)
		else
	...
	if IsInRaid() then
		...
		if raidmember >= 0 then
			...
			for i=1, raidmember do
				BigFoot_DelayCall(function()
					local name,sever = UnitName('raid'..i)
					if not sever and name ~= UNKNOWNOBJECT and UnitIsPlayer(name) and UnitIsConnected(name) and name~=playername then
						SendAddonMessage("BF_VER_CHK",BIGFOOT_VERSION_QUERY,'WHISPER',name)
					end
				end,3)
			end
		end
		...
		if partymember >= 1 then
			...
			for i=1, partymember do
				BigFoot_DelayCall(function()
					local name,sever = UnitName('party'..i)
					if not sever and UnitIsPlayer(name) and UnitIsConnected(name) and name ~= UNKNOWNOBJECT then
						SendAddonMessage("BF_VER_CHK",BIGFOOT_VERSION_QUERY,'WHISPER',name)
					end
				end,3);
			end
