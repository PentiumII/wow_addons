...
function MerInspectConfigFunc()
	...
	if (IsConfigurableAddOn("AutoEquip") or IsConfigurableAddOn("QuickCompare") or IsConfigurableAddOn("RatingBuster") or IsConfigurableAddOn("ReforgeLite")) then
		...
		if (IsConfigurableAddOn("AutoEquip") ) then
			...
			-- ModManagement_RegisterCheckBox(
				...
				-- MOD_INFO_COMPARISON_TIPFRAME_TOOLTIP,
				-- "DisplayStatistics",
				-- 1,
				-- function (arg)
					-- if ( arg == 1 ) then
						-- if (BigFoot_IsAddOnLoaded("MerInspect")) then
							-- MerInspect_ToogleC(true);
						-- end
					-- else
						-- if (BigFoot_IsAddOnLoaded("MerInspect")) then
							-- MerInspect_ToogleC(false);
						-- end
					-- end
				-- end,
				-- 1
		...
		if IsConfigurableAddOn("ReforgeLite") then
			ModManagement_RegisterCheckBox(
				...
				function (arg)
					...
					-- BF_NewAddOnsPromotion("zhCN",307,NewInfoStrName,NewInfoStr,"InfoStat","EnableReforgeLite")
				end
			)
		end
	end
