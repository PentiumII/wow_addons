...
function BagIntegrationConfigFunc()
	...
	if (IsConfigurableAddOn("Combuctor") or IsConfigurableAddOn("PackUpInventory")) then
	...
	local NewInfoStr =[[
	同银行一样，新增的虚空银行也可以在开启【整合背包】这一功能时，对其支持离线查看。
	所在位置：
	大脚设置-背包整合
	]]
	if (IsConfigurableAddOn("Combuctor")) then
		...
		ModManagement_RegisterCheckBox(
			...
			function (arg)
				...
				-- BF_NewAddOnsPromotion("zhCN",307,NewInfoStrName,NewInfoStr,"BagIntegration","Enable_BackPack_Integration")
			end
		);
