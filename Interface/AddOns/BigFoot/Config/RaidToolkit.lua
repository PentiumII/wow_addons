...
function RaidToolkitConfigFunc()
	...
	if (GetLocale() == "zhCN") then
		...
		RaString.Name = "团队报警 ";
		RaidBuilder_ENABLE_TEXT = "开启组队系统";
	...
	elseif (GetLocale() == "zhTW") then
		...
		RaString.Name = "團隊報警 ";
		RaidBuilder_ENABLE_TEXT = "開啟組隊系統";
	...
	if ( IsConfigurableAddOn("Recount") or IsConfigurableAddOn("DBM-Core") or IsConfigurableAddOn("Grid") or IsConfigurableAddOn("RaidAlerter") or IsConfigurableAddOn("Skada") or IsConfigurableAddOn("RaidBuilder") or IsConfigurableAddOn("Omen") or (IsConfigurableAddOn("Decursive") and (playerClass ~= "WARRIOR" and playerClass ~= "ROGUE" and playerClass ~= "DEATHKNIGHT") ) ) then
	...
	if (IsConfigurableAddOn("Decursive") and (playerClass ~= "WARRIOR" and playerClass ~= "ROGUE" and playerClass ~= "DEATHKNIGHT") ) then
		ModManagement_RegisterCheckBox(
			"RaidToolkit",
	...
	if (IsConfigurableAddOn("RaidBuilder")) then
		ModManagement_RegisterCheckBox(
			"RaidToolkit",
			RaidBuilder_ENABLE_TEXT,
			nil,
			"EnableRaidBuilder",
			1,
			function (arg)
				if (arg == 1) then
					...
					if (BigFoot_IsAddOnLoadedFromBigFoot("RaidBuilder")) then
