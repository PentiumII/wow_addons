----------------------------------------------------------------------------------------
--	ChatIcon (Based on ChatEmote by orafy) @ NGA.CC
----------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------
-- Config 		设置
----------------------------------------------------------------------------------------
local Config = {				
	...
	b_always			= true,					-- 表情按钮一直显示
	b_p	= {"BOTTOMLEFT", UIParent, 4, 10},		-- 表情按钮位置
	t_p	= {"BOTTOMLEFT", UIParent, 21, 20},	-- 表情列表位置
}
local fdir = "InterFace\\AddOns\\ChatIcon\\icon\\" -- 表情材质路径
----------------------------------------------------------------------------------------
...
function ChatEmoteButtons()---------------------------------------------------------
	...
	button:SetBackdrop({
		bgFile=fdir.."text_nor_frame", edgeFile="Interface\\Tooltips\\UI-Tooltip-Border",
	...
	button:SetHeight(25)
	button:SetWidth(25)
	if Config.b_always then button:SetAlpha(1) else button:SetAlpha(0) end

-- buttonicon
	local buttontexture = button:CreateTexture(nil, "BORDER")
	buttontexture:SetPoint("CENTER",0,1)
	buttontexture:SetTexture(fdir.."text_nor_icon")  -- ICON
	buttontexture:SetHeight(24)
	buttontexture:SetWidth(24)
	buttontexture:SetAlpha(0.4)
-- click
