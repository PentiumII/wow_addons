...
local COMBUCTOR_VOIDBANK_TEXT,COMBUCTOR_VIEWBUTTOn_TEXT;
...
--	f:SetHeight(settings.h or BASE_HEIGHT)
	...
	if (lastID == 1) then
		local search = _G[name .. "Search"];
		search:ClearAllPoints();
		search:SetPoint("TOPLEFT", name, "TOPLEFT", 160, -44);
		search:SetPoint("TOPRIGHT", name, "TOPRIGHT", -100, -44);
		-------------离线银行和离线虚空银行的下拉菜单--------
