...
--[[	饰品类的相关检测，暂时去掉
do
	local SpellTab = CoolLine:GetSpellData();

	function CoolLine:COMBAT_LOG_EVENT_UNFILTERED(...)
		...
		--------这个事件的参数发生变化。
		local event,hideCaster,sourceGUID,sourceName,sourceFlags,sourceFlags2,destGUID,destName,destFlags,destFlags2,spellID,spellName = select ( 2, ... )
		if sourceGUID == UnitGUID("player") then
			local _,Texture,CDTime,duration;
			-----以下效果分类是根据ExtraCD 这个插件数据推断得出，不确定一定正确---------------------------
			for k,v in pairs(SpellTab)do
				if spellID == k then
					_,_,Texture = GetSpellInfo(k);
					CDTime = v.cd;
					duration = v.duration or 0;
				end
			end

			if Texture and CDTime and duration then
				NewCooldown(spellName, Texture, GetTime()+ CDTime);
			end
		end
	end
end
