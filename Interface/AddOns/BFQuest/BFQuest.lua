...
local function FastQuest_ChangeTitle(questLogTitle, questLogTitleText, level, suggestedGroup, questLogIndex)
	if not questLogTitle then return end
	local LevelString = "["..level.."] ";
	if suggestedGroup > 0 then
		questLogTitle.Text:SetText(" "..LevelString..questLogTitleText.." ("..suggestedGroup..") ");
	else
		questLogTitle.Text:SetText(" "..LevelString..questLogTitleText.." ");
	end
	if ( IsQuestHardWatched(questLogIndex) ) then
		questLogTitle.Check:SetPoint("LEFT", questLogTitle.Text, questLogTitle.Text:GetWrappedWidth() + 2, 0);
	end
end

function lvl:QuestLogQuests_Update(poiTable)
	self.hooks.QuestLogQuests_Update(poiTable);
	for i = 1, #QuestMapFrame.QuestsFrame.Contents.Titles do
		local questLogIndex = QuestMapFrame.QuestsFrame.Contents.Titles[i].questLogIndex
		if questLogIndex then
			local questLogTitleText, level, suggestedGroup = GetQuestLogTitle(questLogIndex);
			local questLogTitle = QuestLogQuests_GetTitleButton(i);
			if questLogTitleText then
				FastQuest_ChangeTitle(questLogTitle, questLogTitleText, level, suggestedGroup, questLogIndex);
			end
		end
	end
end
...
function lvl:OnEnable()
	...
	-- self:RawHook("QuestLogQuests_Update", true);
end

function lvl:OnDisable()
	--hide quest level
	SetCVar("colorblindMode","0")
	-- self:Unhook("QuestLogQuests_Update");
end
