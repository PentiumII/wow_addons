...
function BUnitFrame_OnEvent(self, event, ...)
	...
	elseif (event == "UNIT_MANA" or event == "UNIT_RAGE" or event == "UNIT_FOCUS" or event == "UNIT_ENERGY" or event == "UNIT_MAXMANA" or event == "UNIT_MAXRAGE" or event == "UNIT_MAXFOCUS" or event == "UNIT_MAXENERGY") then
	...
	elseif (event == "UNIT_SPELLCAST_START" or event == "UNIT_SPELLCAST_CHANNEL_START") then
		...
		if (event == "UNIT_SPELLCAST_START") then
			...
			name, nameSubtext, text, texture, startTime, endTime, isTradeSkil = UnitChannelInfo(self.unit);
...
function BUnitFrame_UpdateDebuff(__frame)
	...
	for __i = 1, 4, 1 do
		...
		name, rank, icon, count, debuffType, duration, timeLeft = UnitDebuff(__frame.unit, __i);
		button = __frame.debuff[__i];
		if ( icon ) then
			...
			if ( duration  ) then
				if ( duration > 0 ) then
					...
					CooldownFrame_SetTimer(debuffCooldown, startCooldownTime, duration, 1);
				else
					debuffCooldown:Hide();
				end
			else
