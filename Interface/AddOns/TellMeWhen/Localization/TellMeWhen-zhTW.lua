...
L["ANCHOR_CURSOR_DUMMY"] = "TellMeWhen滑鼠光標"
L["ANCHOR_CURSOR_DUMMY_DESC"] = [=[這是一個虛擬的滑鼠遊標，作用是幫你定位圖示。

在使用”滑鼠遊標目標“單位時，利用該遊標定位群組非常有用。
...
L["CASTERFORM"] = "施法者形態"
L["CENTER"] = "中間"
L["CHANGELOG_INFO"] = [=[歡迎使用TellMeWhen v%s!

你可以花一點點時間來看看這個版本更新了什麼。

如果需要再次開啟這個視窗，請使用下列指令：

/tmw 更新日誌
/tmw changelog

當你準備好編輯你的圖示時請關閉這個視窗。]=]
L["CHANGELOG_MSG"] = "感謝幫忙測試TellMeWhen %s! 輸入/tmw changelog可以查看更新的內容。"
L["CHANGELOG_OKAY_OPENIE"] = "打開圖示編輯器"
L["CHAT_FRAME"] = "聊天視窗"
...
L["ERROR_NOTINITIALIZED_INTERFACEPANEL_EXPLANATION"] = [=[TellMeWhen或TellMeWhen_Options在初始化過程中發生了嚴重的錯誤，當前無法正常使用,也不能進行任何設定。
...
網址：|cff7fffffhttp://wow.curseforge.com/addons/tellmewhen/create-ticket/|r
...
L["EVENTS_INSTRUCTIONS"] = [=[在此選項卡,你可以設定此圖示的額外提醒效果,包括播放音效,動畫和輸出文字.

這些效果將在你所選的事件發生時觸發.大部分事件會在圖示狀態改變後觸發，除此之外也有其他的一些事件，請自行查看說明。

同時它們也會在設定的條件為真（true）時觸發。

點擊左側按鈕開始設定,請按照自己的需要來選擇通知事件和触發效果的組合.]=]
...
L["HELP_SIMPLEGSTAB"] = "點擊切換為\"群組單獨設定“或\"完全設定\"。"
L["ICON"] = "圖示"
...
L["IE_NOLOADED"] = [=[當前沒有載入任何圖示到圖示編輯器。

要加載一個圖示，請用滑鼠右鍵點擊它。

如果你在熒幕上無法看到圖示，輸入/tmw options來顯示你的群組設定。

在那邊你能新增一個群組或者設定一個已存在的群組供你角色當前的職業專精使用。

輸入/tmw或/tellmewhen退出設定模式。]=]
L["IE_SIMPLEGS"] = "完整選項"
L["IE_SIMPLEGS_DESC"] = [=[勾選在這個標籤頁顯示TMW完全的主要選項。

取消勾選僅顯示群組中當前加載的圖示的選項在此標籤頁。]=]
...
L["IMPORT_NEWGUIDS_ICON"] = "文字顯示樣式：%s"
...
L["RESIZE"] = "改變大小"
L["RESIZE_GROUP_CLOBBERWARN"] = "當你使用|cff7fffff右鍵點擊並拖拽|r縮小分組時，部分圖示的設定將會臨時存檔，在你使用|cff7fffff右鍵點擊並拖拽|r放大分組時會恢復，但是在你登出或者重新載入UI後臨時存檔的數據將會丟失。"
L["RESIZE_TOOLTIP"] = "|cff7fffff點擊並拖拽|r以改變大小"
L["RESIZE_TOOLTIP_CHANGEDIMS"] = "|cff7fffff滑鼠右鍵點擊並拖拽|r來更改群組的行跟列的數量。"
L["RESIZE_TOOLTIP_SCALEXY"] = [=[|cff7fffff點擊同時拖拽|r快速調整大小比例
|cff7fffff按住CTRL|r微調大小比例]=]
L["RESIZE_TOOLTIP_SCALEY_SIZEX"] = "|cff7fffff點擊並拖拽|r以調整大小比例"
L["RIGHT"] = "右"
L["ROLEf"] = "職責: %s"
L["Rooted"] = "纏繞"
L["RUNEOFPOWER"] = "符文%d"
...
L["UIPANEL_DELGROUP_DESC2"] = "按住|cff7fffffCtrl|r跳過確認"
L["UIPANEL_DRAWEDGE"] = "高亮計時器指針"
L["UIPANEL_DRAWEDGE_DESC"] = "高亮冷卻計時器指針(時鐘動畫)來突出顯示效果"
L["UIPANEL_DRDURATION"] = "遞減持續時間"
L["UIPANEL_DRDURATION_DESC"] = [=[設定遞減的持續時間。

暴雪官方描述的遞減時間為15秒，但是許多情況下，可能還是太短，會導致你的遞減圖示在顯示遞減結束時，實際上遞減卻還沒有完全結束。你可能要加上1秒或2秒以便遞減圖示正常工作。TellMeWhen的預設時間為17秒。

此選項為帳號共用，所有你的角色都將使用此設定。]=]
...
L["UIPANEL_ENABLEGROUP_FORPROFILE"] = "對角色設定檔%s啟用該群組"
...
L["UIPANEL_TOOLTIP_ENABLEGROUP_GLOBAL_DESC"] = "在這個角色的設定檔中顯示並啓用該群組。更改此設定不會影響你其他角色的設定檔。"
...
L["VALIDITY_ONICONSHOWHIDE_DESC"] = "事件中檢查的第%d個圖示是無效的 >>>"
